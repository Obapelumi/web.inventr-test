<template>
  <div
    class="relative flex w-full flex-col text-sm text-gray-600"
    placeholder=""
  >
    <label
      v-if="label"
    ...
    <input
      ...
      ref="input"
      class="focus:outline-primary-300 h-11 w-full rounded border border-gray-200 py-2 text-gray-500"
    />
    <div class="absolute left-0 h-11 w-fit" ref="prepend">
      <slot name="prepend"></slot>
    </div>
    <div class="absolute right-0 h-11 w-fit" ref="append">
...
// methods
const onInvalid = (e: Event) => (error.value = validateInput(e as InputEvent))
